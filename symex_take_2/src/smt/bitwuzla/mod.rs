...
impl SmtSolver for Bitwuzla {
    ...
    fn new() -> Self {
        ...
        Self {
            ctx: Rc::new(bitwuzla::Bitwuzla::builder().with_model_gen().build()),
        }
    }
...
impl Bitwuzla {
    ...
    pub fn _get_values(
        ...
        expr: &BitwuzlaExpr,
        upper_bound: usize,
    ) -> Result<Solutions<BitwuzlaExpr>, SolverError> {
        let expr = expr.clone().simplify();
        if expr.get_constant().is_some() {
...
impl SmtExpr for BitwuzlaExpr {
    ...
    fn get_constant(&self) -> Option<u64> {
        self.0
            .as_binary_str()
            .as_ref()
            .map(|word| u64::from_str_radix(word, 2).ok())?
...
impl ArrayMemory {
    pub fn resolve_addresses(
        ...
        addr: &BitwuzlaExpr,
        _upper_bound: usize,
    ) -> Result<Vec<BitwuzlaExpr>, MemoryError> {
        Ok(vec![addr.clone()])
    }
    ...
    fn internal_read(
        ...
        bits: u32,
        ptr_size: u32,
    ) -> Result<BitwuzlaExpr, MemoryError> {
        let value = if bits < BITS_IN_BYTE {
            ...
            for byte in 0..num_bytes {
                let offset =
                    BitwuzlaExpr(BV::from_u64(self.ctx.clone(), byte as u64, ptr_size as u64));
    ...
    fn internal_write(
        ...
        for n in 0..num_bytes {
            ...
            let offset = match self.endianness {
                Endianness::Little => {
                    BitwuzlaExpr(BV::from_u64(self.ctx.clone(), n as u64, ptr_size as u64))
                }
                Endianness::Big => BitwuzlaExpr(BV::from_u64(
                    self.ctx.clone(),
                    (num_bytes - 1 - n) as u64,
                    ptr_size as u64,
                )),
            };
...
impl SmtMap for BitwuzlaMemory {
    ...
    fn from_u64(&self, value: u64, size: usize) -> Self::Expression {
        assert!(size != 0, "Tried to create a 0 width constained value");
        BitwuzlaExpr(BV::from_u64(
            self.ram.ctx.clone(),
            value & ((1 << size) - 1),
            size as u64,
        ))
...
mod test {
    ...
    use crate::{
        ...
        defaults::bitwuzla::DefaultComposition,
        executor::{
            add_with_carry,
            count_leading_ones,
