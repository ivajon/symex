...
pub fn run_elf<P: AsRef<Path>>(
    ...
    match arch {
        SupportedArchitechture::ArmV7EM(v7) => {
            ...
            let project = Box::new(Project::from_path(&mut cfg, obj_file, &v7)?);
            let project = Box::leak(project);
            project.add_pc_hook(end_pc, PCHook::EndSuccess);
            debug!("Created project: {:?}", project);

            let mut vm = VM::new(project, context, function, end_pc, v7)?;
        ...
        SupportedArchitechture::ArmV6M(v6) => {
            let mut cfg = RunConfig::new(show_path_results);
            add_architecture_independent_hooks(&mut cfg);
            let project = Box::new(Project::from_path(&mut cfg, obj_file, &v6)?);
            let project = Box::leak(project);
            project.add_pc_hook(end_pc, PCHook::EndSuccess);
            debug!("Created project: {:?}", project);

            let mut vm = VM::new(project, context, function, end_pc, v6)?;
            run_elf_paths(&mut vm, &cfg)
        }
    }
}
...
pub fn run_elf_configured<A: Architecture>(
    ...
    add_architecture_independent_hooks(&mut cfg);
    let project = Box::new(Project::from_path(&mut cfg, obj_file, &architecture)?);
    let project = Box::leak(project);
    project.add_pc_hook(end_pc, PCHook::EndSuccess);
    debug!("Created project: {:?}", project);
...
fn run_elf_paths<A: Architecture>(
    ...
    while let Some((path_result, state)) = vm.run()? {
        ...
        let v_path_result = match path_result {
            PathResult::Success(_) => PathStatus::Ok(None),
            PathResult::Failure(reason) => PathStatus::Failed(ErrorReason {
                error_message: reason.to_owned(),
            }),
            PathResult::AssumptionUnsat => todo!(),
            PathResult::Suppress => todo!(),
        };
